...
function ChatBubble({
  ...
  const preprocessMarkdown = (text: string): string => {
    ...
    // 第一步：处理数字编号列表（如 "1. "、"2. "等）
    // 将行内的数字编号列表项分离出来
    let processed = text
      // 处理连续的数字编号列表（如 "1. xxx 2. yyy"），在数字编号前添加换行
      .replace(/(\d+\.\s+[^0-9\n]+?)(\s+)(\d+\.\s+)/g, '$1\n$3')
      // 处理 " 1. " 格式（前面有空格，后面跟着内容）
      .replace(/([^\n])\s+(\d+\.\s+)/g, '$1\n$2')
      // 处理行首的数字编号（确保前面有换行）
      .replace(/([^\n])(\d+\.\s+)/g, '$1\n$2')
      // 确保数字编号列表前后都有空行（段落和列表之间）
      .replace(/([^\n])\n(\d+\.\s+)/g, '$1\n\n$2')
      // 数字列表项结束后如果有非列表内容，添加空行
      .replace(/(\d+\.\s+[^\n]+)\n([^\n\d])/g, '$1\n\n$2');
    ...
    // 第三步：清理多余的连续换行（最多保留两个）
    processed = processed.replace(/\n{3,}/g, '\n\n').trim();
    
    // 第四步：处理分类标题（首字母大写，不包含引号）
